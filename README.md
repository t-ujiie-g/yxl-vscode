...
| **Applies a look** | Bold, italic, underline, strike, a fill, a text colour, both alignment axes, wrap and a number format — from the toolbar, over a cell or a selection, each with a way to take it off again — including on a cell that reads it from a band, which the spec now says with `null`. What is written is the normalizer's answer: a declaration that already says it, a variant extending the one the cell wears, or the look itself — never a fourth anonymous copy of the same thing. Where the look comes from a shared declaration or a column band, you are asked, with the count of what each answer would move. Over a selection whose cells take it from different places, the two answers are apply it to all of them alike, or change each place it comes from — never a silent pick, and not a question at all where both would leave the file the same. |
