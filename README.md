...
| **Refuses the rest, out loud** | A value that comes from a definition, a CSV, a parameter, or a `formulas:` range has more than one answer or none, so the preview says which — under the grid, where you typed — rather than guessing or going quiet. Picking between those answers is the next phase. |
| **…and offers the exception** | Every refusal about a real cell carries `overrides:`, the exception yxl has for exactly this (`docs/spec.md` §23): say why, and the cell is written as one deliberate exception with your reason beside it, marked in the grid ever after. Offered, never taken on its own. |
