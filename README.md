...
| **Tidies the spec, where that provably changes nothing** | **yxl: Gather Repeated Looks into a Definition** reads the whole spec and offers what analysis can see: a look written out in full at three places or more becomes one `defs.styles` entry; definitions that resolve alike become one the rest follow; a column of cells each translating one formula becomes the `formulas:` range that fills them. Every one of them **claims to change no rendered cell, and is refused automatically if it would** — so the proposal's provenance stops mattering. It is shown as a diff in VS Code's own diff editor and written only on the answer after it, never silently. |
...
needs nothing. This editor targets **yxl 0.5.1** — an older or newer compiler is
