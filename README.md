...
| **…and offers the answers, where it has them** | A formula typed into the cell a `formulas:` range is anchored at is offered as *the range's* formula, with how many cells that would change and a few of them by name. You pick; nothing is picked for you. The other origins get their answers as the resolution table lands row by row. |
