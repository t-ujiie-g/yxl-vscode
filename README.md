...
| **Draws and writes an outline** | `group:` shows as a bar along the headings of the run, one level in from the last, with the control at its end. Collapsing writes it — `group` with `hidden: true` is what the schema calls a collapsed group — so what you see is what the file says, and the `+` that opens one is on the heading the run hides behind. |
