...
| **Refuses the rest, out loud** | A value that comes from a definition, a CSV, a parameter, or a `formulas:` range has more than one answer or none. Those cells carry a grey corner mark and say so when selected — before you type, not after — and typing anyway gets the reason. Picking between the answers is the next phase. |
