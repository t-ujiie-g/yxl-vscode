...
/** A sheet four cells square, which is as much as a clipboard rectangle needs. */
function sheet(cells: readonly DrawnCell[], of: Partial<DrawnSheet> = {}): DrawnSheet {
  return drawnSheet({ rows: 4, columns: 4, of: { rows: 4, columns: 4 }, cells, ...of });
}
...
describe('what a copied rectangle puts on the clipboard', () => {
  const GRID = sheet([
    cell(1, 1, { row: 1, col: 1, value: 'APAC' }),
    cell(1, 1, { row: 1, col: 2, value: 2400000 }),
    cell(1, 1, { row: 2, col: 1, value: 'EMEA' }),
    cell(1, 1, { row: 2, col: 2, value: 1750000 }),
  ]);
  ...
  it('leaves a cell nothing writes empty on both sides', () => {
    const sparse = sheet([cell(1, 1, { row: 1, col: 1, value: 'APAC' })]);
  ...
  it('carries the value in the text and the format in the table (ADR-028)', () => {
    const shown = sheet([cell(1, 1, { row: 1, col: 1, value: 1234.5, format: '#,##0.00' })]);
  ...
  it('writes a colour as hex, which is the form the other spreadsheets read', () => {
    ...
    const styled = sheet([cell(1, 1, { row: 1, col: 1, style: { fill: '80FF0000' as Color } })]);
  ...
  it('carries a computed result rather than the formula it came from', () => {
    const computed = sheet([
      cell(1, 1, {
        ...
        col: 1,
        formula: 'SUM(B1:B2)',
        computed: { kind: 'value', value: 4150000 },
      }),
    ]);
  ...
  it('carries an uncomputed formula as the formula, which is all there is of it', () => {
    const uncomputed = sheet([cell(1, 1, { row: 1, col: 1, formula: 'TODAY()' })]);
  ...
  it('carries a filled cell its own formula, which is what the workbook holds there', () => {
    const filled = sheet([cell(1, 1, { row: 1, col: 1, formula: 'A1*2', filledFrom: 'C2' })]);
  ...
  it('quotes a field holding what a row or a field ends on', () => {
    const awkward = sheet([
      cell(1, 1, { row: 1, col: 1, value: 'a\tb' }),
      cell(1, 1, { row: 1, col: 2, value: 'say "hi"' }),
    ]);
  ...
  it('escapes what the table cannot hold as itself', () => {
    const marked = sheet([cell(1, 1, { row: 1, col: 1, value: '<b> & "x"' })]);
  ...
  it('says nothing at all where the rectangle reaches past what was drawn', () => {
    const window = sheet([cell(1, 1, { row: 1, col: 1, value: 'APAC' })], {
      rows: 2,
      columns: 2,
